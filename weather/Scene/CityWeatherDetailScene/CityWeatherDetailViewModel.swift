    ...
    var city:CityWeatherModel?{
        didSet{
           let rx_city = Observable.from(object: self.city!)
            ...
            rx_city.map {"\($0.temperature) ℃"}.bind(to: rx_temperature).disposed(by: disposeBag)
            rx_city.map {"\($0.windSpeed) km/h"}.bind(to: rx_windSpeed).disposed(by: disposeBag)
            rx_city.map {$0.conditionText ?? ""}.bind(to: rx_conditionText).disposed(by: disposeBag)
            rx_city.map {URL(string:$0.conditionIcon ?? "")}.bind(to: rx_conditionIcon).disposed(by: disposeBag)
            rx_city.map {$0.updateTime?.standardText() ?? ""}.bind(to: rx_updateTime).disposed(by: disposeBag)
